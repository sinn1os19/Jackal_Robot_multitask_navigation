...
def find_nearest_box(new_point, box_positions):
    """计算新点与每个 box 点之间的欧氏距离，返回最小距离的 box 编号及距离"""
    nearest_box = None
    min_distance = float('inf')
    for box_id, pos in box_positions.items():
        # 计算欧氏距离
        dx = new_point[0] - pos[0]
        dy = new_point[1] - pos[1]
        distance = math.sqrt(dx*dx + dy*dy)
        if distance < min_distance:
            min_distance = distance
            nearest_box = box_id
    return nearest_box, min_distance
        ...
        for cnt in contours:
            ...
            if recognized_digit is not None:
                ...
                if recognized_digit == self.min_digit:
                    ...
                    target_point = (cube_point_map.point.x, cube_point_map.point.y)

                    nearest_box, distance = find_nearest_box(target_point, box_positions)

                    cube_position = {"x": box_positions[nearest_box][0], "y": box_positions[nearest_box][1]}
