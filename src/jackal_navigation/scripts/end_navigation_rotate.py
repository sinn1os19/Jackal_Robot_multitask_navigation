...
class BoxNavigation:
    ...
    def crossed_bridge_callback(self, msg):
        """订阅 /crossed_bridge，当收到 True 时开始导航，并注销订阅"""
        if msg.data:
    ...
    def go_to_position(self, x, y, yaw=math.pi):
    ...
    def target_cube_callback(self, msg):
        ...
        try:
            ...
            if "position" in data:
                ...
                self.target_cube_sub.unregister()
    ...
    def rotate_by_angle(self, angle, angular_speed=0.5):
        """
        旋转机器人指定角度（以弧度为单位），在旋转过程中检测目标立方体，
        如果检测到则返回 "detected"，否则旋转完成后返回 "completed"
        """
        start_x, start_y, start_yaw = self.get_current_pose()
        if start_yaw is None:
            rospy.logerr("无法获取初始朝向，旋转失败")
            return "failed"
        desired_yaw = self.normalize_angle(start_yaw + angle)
        rate = rospy.Rate(10)
        twist = Twist()
        twist.linear.x = 0.0
        twist.angular.z = angular_speed if angle > 0 else -angular_speed

        rospy.loginfo("开始旋转 %.2f 弧度，目标朝向: %.2f", angle, desired_yaw)
        while not rospy.is_shutdown():
            _, _, current_yaw = self.get_current_pose()
            angle_remaining = self.normalize_angle(desired_yaw - current_yaw)
            if abs(angle_remaining) < 0.05:  # 阈值：旋转完成
                break
            self.cmd_vel_pub.publish(twist)
            # 检查是否检测到目标立方体
            if self.target_cube_position is not None:
                rospy.loginfo("在旋转过程中检测到目标立方体")
                twist.angular.z = 0.0
                self.cmd_vel_pub.publish(twist)
                return "detected"
            rate.sleep()
    ...
    def new_navigation(self):
        """
        新的导航逻辑：
        1. 获取起始位置 (x0, y0)
        2. 根据 y0 值决定旋转方向：
           - 如果 y0 < -12：先向右旋转60度，再向左旋转120度
           - 如果 y0 > -12：先向左旋转60度，再向右旋转120度
           在旋转过程中如果检测到目标立方体，则立即停止旋转，并导航至目标立方体前方 (target_x, target_y)
        3. 如果上述旋转过程中未检测到目标立方体，则导航至 (5.0, -12.0)，然后反向旋转180度检测目标立方体，
           如果检测到，则导航至目标立方体前方 (target_x, target_y)；否则输出未检测到立方体的日志。
        """
        current_x, current_y, current_yaw = self.get_current_pose()
        if current_x is None:
            rospy.logerr("无法获取当前位姿，导航中止")
            return False

        rospy.loginfo("起始位置: (%.2f, %.2f)", current_x, current_y)
        # 清空之前的目标立方体数据
        # self.target_cube_position = None

        # 根据 y0 值判断旋转策略
        if current_y < -12:
            rospy.loginfo("y0 < -12，先向右旋转60度")
            result = self.rotate_by_angle(-math.pi/3)  # -60度
            if result == "detected":
                target_x = self.target_cube_position["x"]
                target_y = self.target_cube_position["y"]
                rospy.loginfo("检测到目标立方体，直接导航至前方 (%.2f, %.2f)", target_x, target_y)
                self.go_to_position(target_x, target_y)
                return True
            rospy.loginfo("再向左旋转120度")
            result = self.rotate_by_angle(2*math.pi/3)  # +120度
            if result == "detected":
                target_x = self.target_cube_position["x"]
                target_y = self.target_cube_position["y"]
                rospy.loginfo("检测到目标立方体，直接导航至前方 (%.2f, %.2f)", target_x, target_y)
        ...
        # 如果旋转过程中未检测到目标立方体，则导航至 (5.0, -12.0)
        rospy.loginfo("旋转过程中未检测到目标立方体，导航至 (5.0, -12.0)")
        if not self.go_to_position(5.0, -12.0):
            rospy.logerr("无法导航至 (5.0, -12.0)")
            return False

        rospy.loginfo("反向旋转180度检测目标立方体")
        result = self.rotate_by_angle(math.pi)  # 180度
        if result == "detected":
            target_x = self.target_cube_position["x"]
            target_y = self.target_cube_position["y"]
            rospy.loginfo("检测到目标立方体，直接导航至前方 (%.2f, %.2f)", target_x, target_y)
            ...
            rospy.loginfo("旋转后仍未检测到目标立方体")
            rospy.loginfo("未检测到立方体")
            return False
